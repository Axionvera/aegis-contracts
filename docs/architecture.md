...
* **`compliance.rs`:** Owns the investor compliance lifecycle — a five-state machine (`Unknown`, `Pending`, `Approved`, `Revoked`, `Blocked`) with an enforced transition matrix, role-gated writes, and a `compliance_status_changed` event on every transition. Exposes `require_can_send` / `require_can_receive`, which `asset.rs` consumes on every mint and transfer. Privileged operations require the ComplianceOfficer role; lifting a `Blocked` state requires the supreme admin. See [Compliance Status Lifecycle](compliance-lifecycle.md).
...
* **`eligibility.rs`:** Read-only composition of the compliance lifecycle, asset lifecycle, holding-cap, and pause state into per-investor eligibility answers. Introduces no storage of its own.
...
| ComplianceOfficer | set_compliance_status (except leaving `Blocked`), whitelist_user, revoke_whitelist |
...
* **What remains available:** read functions (`get_role_of`, `get_balance_of`, `get_total_supply`, `is_whitelisted`, `get_compliance_status`, `is_compliance_transition_allowed`, `get_allowed_transitions`, `get_allowed_transitions_for`, `is_paused`, `get_investor_eligibility`, `check_transfer_eligibility`, `get_capabilities`, `supports_capability`, `get_capability_keys`)
...
* **Persistent Storage:** `Role` assignments, `ComplianceStatus` lifecycle state (with the legacy `Whitelist` boolean maintained as a derived mirror), and User `Balance`. These must persist independently and be rent-exempted appropriately to ensure user balances and roles are never archived unexpectedly.
